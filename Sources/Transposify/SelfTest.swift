    ...
    static func run(_ controller: AudioController) {
        ...
        let steps: [Step] = [
            ...
            Step(label: "Back to 0 \u{2192} disengage (debounced)",
                 ...
                 expectEngaged: false, expectSemitones: 0),
            Step(label: "Reduce vocals on at 0 \u{2192} engage",
                 delay: 0.05,
                 action: { controller.setVocalReduction(.fast) },
            ...
            Step(label: "Resume \u{2192} re-engage (reduction still on)",
                 delay: 0.05,
                 action: { controller.spotifyUpdate(running: true, playing: true, trackID: trackA) },
                 expectEngaged: true, expectSemitones: nil),
            Step(label: "Set +5 on A (reduction off), remembered",
                 delay: 0.05,
                 action: { controller.setVocalReduction(.off); controller.setSemitones(5) },
